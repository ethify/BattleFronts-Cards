...
export const startGame = () => {
    for (var i=0;i<4;i++){
        const newUserCards = drawOneCard(userGame.game_data.deck_player, userGame.game_data.hand_player)
        const newAICards = drawOneCard(userGame.game_data.deck_ai, userGame.game_data.hand_ai)
...
export const playCard = (playerCardId) => {
    ...
    userGame.game_data.selected_card_player = userGame.game_data.hand_player[playerCardId]
    userGame.game_data.hand_player[playerCardId] = 0

    const aiCardID = Math.floor(Math.random() * userGame.game_data.hand_ai.length)
...
export const updateGameStatus = () => {
    if (userGame.game_data.life_ai <= 0) {
        ...
        const playerFinished = userGame.game_data.hand_player.filter(card => card === 0).length === 4
        const aiFinished = userGame.game_data.hand_ai.filter(card => card === 0).length === 4
...
export const nextRound = () => {
    ...
    if ( (userGame.game_data.selected_card_player === 0 || userGame.game_data.selected_card_ai === 0) ){
